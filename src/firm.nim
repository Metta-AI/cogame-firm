## Firm entrypoint: reads the Coworld runtime contract and starts either a
...
proc randomSeed(): int =
  ...
  if not urandom(buf):
    raise newException(FirmError, "OS entropy source unavailable")
...
when isMainModule:
  ...
  if runtimeConfig.replayMode:
    ...
    if not seedPinned(runtimeConfig.config):
      ## An unpinned seed is randomized so the role permutation, the order
      ## board and the aliases are not precomputable.
      config.seed = randomSeed()
      echo "firm: seed not pinned; randomized"
    ## Fit the cap AFTER the seed is settled, so a pinned seed reproduces
    ## the episode exactly.
    config = sampleEpisode(config)
    echo "firm: seats=", config.players.len,
